...
## Circuit Discovery (Attribution Patching)

Code to discover minimal circuits at the level of edges, nodes, or neurons via attribution patching (EAP-IG) on ESM3 and ESM-C. Attribution patching scores components by how much they affect the model's output; the pipeline runs a binary search to find the minimal circuit that reproduces full-model behavior above a faithfulness threshold. Uses circuit_discovery datasets from the counterfactual experiment. Run with e.g. `python scripts/attribution_patching_experiment/run.py --graph_type nodes --methods blosum --repeat_types identical --model_types esm3`. See [scripts/attribution_patching_experiment/README.md](scripts/attribution_patching_experiment/README.md) for full usage and commands to produce all circuits from the paper.
