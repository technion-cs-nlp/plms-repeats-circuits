...
def are_amino_acids_blosum_similar(aa1, aa2):
    """
    ...
    for rep_idx, (start, end) in enumerate(repeat_locations):
        ...
        for pos in range(start, end + 1):
            ...
            position_data = RepeatPositionData(
                ...
                is_non_aligned_matching_similiar= are_amino_acids_blosum_similar(curr_aa, other_nonaligned_abs_matched_aa) if other_nonaligned_abs_matched_aa else False,
                ...
                is_aligned_matching_similiar= are_amino_acids_blosum_similar(curr_aa, align_match_aa) if align_match_aa and align_match_aa not in ['.', '-'] else False,
...
def test_analyze_repeat_positions_substitution_and_gap():
    protein_seq = "ABCDEFGKACDELGPO"

    # Define repeat locations
    repeat_locations = [[0, 6], [8, 13]]  # "ABCDEFG" and "ACDELG"

    # Manual alignment:
    aligned_repeat0 = "ABCDEFG"
    aligned_repeat1 = "A.CDEL" + "G"  # Length must match

    alignments = [aligned_repeat0, aligned_repeat1]

    result_abs, result_align = analyze_repeat_positions(protein_seq, repeat_locations, alignments)

    print("=== REPEAT ANALYSIS ===")
    print("=== ABSOLUTE POSITION MAP ===")
    for rep_idx in result_abs:
        print(f"\nRepeat {rep_idx}:")
        for abs_pos, data in result_abs[rep_idx].items():
            print(f"  Abs Pos {abs_pos}: {data}")

    print("\n=== ALIGNMENT POSITION MAP ===")
    for rep_idx in result_align:
        print(f"\nRepeat {rep_idx}:")
        for align_pos, data in result_align[rep_idx].items():
            print(f"  Align Pos {align_pos}: {data}")

    print("\n=== CHECK: Substitution F→L ===")
    f_abs_pos = 5  # 'F' in repeat 0
    data = result_abs[0][f_abs_pos]
    assert data.identity == 'F'
    assert data.aligned_matching_identity == 'L'
    assert data.is_aligned_matching_identical == False
    assert data.non_aligned_matching_identity == 'G'
    assert data.is_nonaligned_matching_identical == False
    print(f"Substitution correctly detected: {data.identity} → {data.aligned_matching_identity} ❌")

    print("\n=== CHECK: Gap at 'B' position ===")
    b_abs_pos = 1  # 'B' in repeat 0
    data = result_abs[0][b_abs_pos]
    assert data.identity == 'B'
    assert data.aligned_matching_identity == '.'
    assert data.is_aligned_matching_identical == False
    print(f"Gap correctly detected at position of 'B': {data.identity} → GAP (.)")

    print("\nAll checks passed ✅")

# # Run it
# if __name__ == "__main__":
#     test_analyze_repeat_positions_substitution_and_gap()
