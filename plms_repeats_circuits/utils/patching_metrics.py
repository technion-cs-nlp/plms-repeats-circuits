...
def logit_diff(logits: torch.Tensor, clean_logits: torch.Tensor, masked_positions: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
...
def log_prob(logits: torch.Tensor, clean_logits: torch.Tensor, masked_positions: torch.Tensor, labels: torch.Tensor, mean=True, loss=False):
    masked_clean_logits = get_masked_token_logits(logits, masked_positions)
    if masked_clean_logits.dim() != 2:
        raise ValueError(f"Expected masked_clean_logits to have shape (batch_size, hidden_dim), but got {masked_clean_logits.shape}")
    if labels.dim() == 1:
